...
/// Portable SHA-256 (FIPS 180-4) — CryptoKit is Apple-only and this module
/// also builds on Linux. Used to compare local mirror files with the
/// remote `sha256sum`/`shasum -a 256` output, so it must match them bit
/// for bit.
enum SHA256 {
    ...
    static func hex(ofFile url: URL) -> String? {
