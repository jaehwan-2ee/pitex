...
export async function verifyTargetDag({ root = DEFAULT_ROOT } = {}) {
  ...
  const required = ['TexDomain','ProjectCore','DocumentSessionCore','LanguageCore','BuildCore','SyncTeXCore','AICore','ParityKit','AppPorts','ProjectFeature','EditorFeature','EditorMacAdapter','BuildFeature','PDFFeature','SettingsFeature','MacPlatform','AppShell'].sort();
  if (JSON.stringify(names) !== JSON.stringify(required)) add(violations, 'DAG_TARGET_SET', 'Tools/target-dag.json', 'target set does not exactly match the approved 17 targets');
